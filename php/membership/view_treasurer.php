...
*/
  ...
  foreach($res as $rs){
    ...
    if (!isset($_GET['iscollector'])) {
      $btn = '
              <button class="btn btn-sm btn-primary" title="Deposit" onclick="deposit_details(\''.base64_encode($id).'\',\''.base64_encode(strtoupper($name)).'\')"><i class="material-icons md-11 align-middle" style="font-size:15px!important;">branding_watermark</i>
              </button>';
    }
    ...
    $data[] = array(
            ...
            "status"=>'<input type="checkbox" id="st'.$id.'" name="bootstrap-switch" data-off-color="danger" '.$st.' disabled>',
            "action"=>'
              <button class="btn btn-sm btn-info" title="View Bill" onclick="bill_details(\''.base64_encode($id).'\',\''.base64_encode(strtoupper($name)).'\')"><i class="material-icons md-11 align-middle" style="font-size:15px!important;">attach_money</i>
              </button>
            ' . $btn
        );
  }
