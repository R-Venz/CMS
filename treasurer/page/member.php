  ...
  <!-- <div class="modal-dialog" role="document"> -->
    <div class="modal-content">
      <div class="modal-header">
        ...
        <!-- <div class="modal-header"> -->
            <!-- <div class="col-lg-4"> -->
            <!-- <input type="button" name="" value="Deposit" class="btn-primary btn btn-outine btn-sm" onclick="deposit(thisID);"> -->
...
<script>
    ...
    function view() {
        ...
        var empDataTable = $('#user_list').DataTable({
              ...
              'serverMethod': 'post',
              'ajax': {
                  'url':'<?php echo web_root; ?>php/membership/view_treasurer.php?sr='+$('#txtsearch').val(),
    ...
    function view_member() {
        ...
        autoSizeDatatableInModal('modal_members');
    }
    ...
    function view_bill() {
        ...
        var DataTable = $('#bill_history').DataTable({
                ...
                'serverMethod': 'post',
                'ajax': {
                  'url':'<?php echo web_root; ?>php/bill/view.php?id='+thisID,
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.log(jqXHR)
                        Swal.fire('',errorThrown,'error')
                    }
    ...
    function view_deposit() {
        ...
        var DataTable = $('#bill_history').DataTable({
                ...
                'serverMethod': 'post',
                'ajax': {
                  'url':'<?php echo web_root; ?>php/deposit/view.php?id='+thisID,
                    error: function (jqXHR, textStatus, errorThrown) {
                        console.log(jqXHR)
                        Swal.fire('',errorThrown,'error')
                    }
    ...
    function deposit_details(id,name) {
        thisID = id;
        $('#modal_history .modal-title').html(decodestr(name));
        view_deposit();
        $('#modal_history').modal('toggle');
    ...
    function deposit(id) {
        Swal.fire({
            title: 'Deposit',
            html: "<input type='number' id='deposit_amt' min='1' max=''>",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#197920',
            confirmButtonText: 'Confirm',
            ...
            if (res.isDismissed) {
                ...
                postData.append('id', decodestr(id));
                postData.append('amt', $('#deposit_amt').val());
                $.ajax({  
                    url:"<?=web_root?>php/deposit/save.php",  
                    method:"POST",  
                    data:postData,  
                    contentType:false,  
                    //cache:false,  
