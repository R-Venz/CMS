<?php
require_once(LIB_PATH.DS."dbConfig/dbConfig.php");
class Database {
	var $sql_string = '';
	var $error_no = 0;
	var $error_msg = '';
	...
	function  anti_xss($str) {
		...
		// str
		$unsafe = false;
	    $XML = '<root><element>'.$str.'</element></root>';
	...
	public function escape_value( $value ) {
		if( $this->real_escape_string_exists ) { // PHP v4.3.0 or higher
			...
			if($this->magic_quotes_active) { $value = stripslashes($this->anti_xss($value)); }
			$value = mysqli_real_escape_string($this->conn,$this->anti_xss($value));
		} else { // before PHP v4.3.0
			// if magic quotes aren't already on then add slashes manually
			if( !$this->magic_quotes_active ) { $value = addslashes($this->anti_xss($value)); }
