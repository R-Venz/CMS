...
class PDF_LineGraph extends PDF_MC_Table {
    function LineGraph($w, $h, $data, $options='', $colors=null, $maxVal=0, $nbDiv=4){
        ...
        *******************************************/
        ...
        if(strstr($options,'kB')){
            // $this->Line(30+1,$keyY+$lineh/2+$n*$lineh + 80,30+8,$keyY+$lineh/2+$n*$lineh + 80);
            // $this->Rect(10,$keyY+$lineh/2*$lineh + 68,$keyW,$keyH);
        }
        ...
        //draw vertical lines
        // echo count($data[$keys[0]]); return;
        if (count($data[$keys[0]])-1 <= 0) {
            echo "No data to present";return;
        }
        ...
        foreach($data as $key => $value){
            ...
            // $this->Line(30+1 + $legendXGap,($keyY+$lineh/2+$n*$lineh) + 80 - $legendYGap,30+8+ $legendXGap,($keyY+$lineh/2+$n*$lineh) + 80 - $legendYGap);
            $this->Line(10+1 + $legendXGap,(60 + $lineh/2+$n*$lineh) + 80 - $legendYGap,10+8+ $legendXGap,(60 + $lineh/2+$n*$lineh) + 80 - $legendYGap);
            $this->SetXY(20 + $legendXGap,(60 + $n*$lineh) + 80 - $legendYGap);
        ...
        foreach($valueKeys as $key => $value){
            ...
            if (count($valueKeys) == $cnt) $this->TextWithRotation($graphX+$graphW - 5 ,$graphValY+10, $value, 45, -45);
            else $this->TextWithRotation($graphValX+$key*$horiDivW - 5 ,$graphValY+10, $value, 45, -45);
            $cnt++;
        }
