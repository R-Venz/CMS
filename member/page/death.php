  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modal_userLabel">Death Details</h5>
...
<!-- Modals -->
<div class="modal fade" id="modal_status" tabindex="-1" role="dialog" aria-labelledby="modal_userLabel" aria-hidden="true" style="z-index: 999999!important;">
  <!-- <div class="modal-dialog" role="document" style="max-width: 80%;"> -->
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modal_userLabel">Death Status</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<script>
    ...
    function view() {
        ...
        var empDataTable = $('#user_list').DataTable({
              ...
              'serverMethod': 'post',
              'ajax': {
                  'url':'<?php echo web_root; ?>php/death/view.php?sr='+$('#txtsearch').val()+'&ismember',
    ...
    function selectStatus(id,status){
        var opt = '';
        thisID = id;
        if (decodestr(status) == 'Pending') {
            opt += '<option value="Posted">Post</option>';
            opt += '<option value="Rejected">Reject</option>';
        }else if (decodestr(status) == 'Posted') {
            opt += '<option value="Closed">Close</option>';
        }
        if (decodestr(status) == 'Pending' || decodestr(status) == 'Posted') {
            $('#modal_status').modal('toggle');
        }else{
            Swal.fire('You cannot change this record','','warning');
            return;
        }
        document.getElementById('status').innerHTML = opt;
    }
    $('#status_form').on('submit', function(e){
        ...
        postData.append('id', decodestr(thisID));
        $.ajax({  
            url:"<?=web_root?>php/death/savestatus.php",  
            method:"POST",  
            data:postData,  
            contentType:false,  
            //cache:false,  
